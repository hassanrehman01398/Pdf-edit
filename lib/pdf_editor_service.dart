...
class _PdfFileHandler {
  static Future<File> getFileFromAssets(String filename) async {
    ...
    var absoluteName = '${(Platform.isIOS ? await getLibraryDirectory() : await getApplicationDocumentsDirectory()).path}/$name';
    print(absoluteName);
...
class PdfMutableDocument {
  ...
  Future<void> save({String filename}) async {
    ...
    Printing.sharePdf(bytes:build().save(), filename: 'test_pdf.pdf');
  }
}
